# PC-98/IBM PC Web Patcher 구현 계획

## 1. 목표

GitHub Pages에서 실행되는 정적 웹 패처를 만든다. 사용자가 PC-98 또는 IBM PC 게임 폴더를 선택하면 브라우저 안에서 여러 대상 파일을 검사하고 VCDIFF/xdelta 패치를 적용한 뒤, 검증된 결과를 ZIP 파일로 내려받을 수 있어야 한다.
...
PC-98과 IBM PC 게임의 파일명 특성을 고려하여 표시 경로는 원래 표기를 유지한다. Windows 대상 폴더 매칭에서는 대소문자 차이를 허용하되, 대소문자만 다른 중복 파일이 발견되면 안전을 위해 중단한다.
